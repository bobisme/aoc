...
def run_python(filepath: Path) -> list[str] | None:
    ...
    cmd = ["python", str(filepath.name)]
    try:
        result = subprocess.run(
            cmd, capture_output=True, text=True, timeout=60, cwd=filepath.parent
        )
...
def run_rust(filepath: Path, base: str) -> list[str] | None:
    ...
    try:
        result = subprocess.run(
            [exe], capture_output=True, text=True, timeout=60, cwd=filepath.parent
        )
        if result.returncode != 0:
            return None
        return result.stdout.strip().split("\n")
...
def run_zig(filepath: Path, base: str) -> list[str] | None:
    ...
    try:
        result = subprocess.run(
            ["zig", "run", str(filepath.name), "-O", "ReleaseFast", "--name", base],
            capture_output=True,
            text=True,
            timeout=60,
            cwd=filepath.parent,
...
def run_nim(filepath: Path) -> list[str] | None:
    ...
    try:
        result = subprocess.run(
            ["nim", "r", "-d:release", str(filepath.name)],
            capture_output=True,
            text=True,
            timeout=60,
            cwd=filepath.parent,
...
def run_c(filepath: Path, base: str) -> list[str] | None:
    """Compile and run a C solution."""
    exe = f"/tmp/{base}"
    libb_dir = filepath.parent.parent / "libb" / "src"

    # Compile: clang -std=c23 -O2 -I../libb/src ../libb/src/*.c -o exe file.c
    libb_sources = list(libb_dir.glob("*.c"))
    compile_cmd = [
        "clang",
        "-std=c23",
        "-O2",
        f"-I{libb_dir}",
        *[str(s) for s in libb_sources],
        str(filepath),
        "-o",
        exe,
    ]

    compile = subprocess.run(compile_cmd, capture_output=True)
    if compile.returncode != 0:
        return None
...
def run_solution(
    ...
    # Check which files exist (now under year subdirectory)
    py_file = year_dir / f"{base}.py"
    rs_file = year_dir / f"{base}.rs"
    zig_file = year_dir / f"{base}.zig"
    nim_file = year_dir / f"nim_{year}_{padded}.nim"
    c_file = year_dir / f"{base}.c"
    ...
    if (lang_filter is None or lang_filter == "c") and c_file.exists():
        output = run_c(c_file, base)
        if output:
            all_results.extend(parse_output(output, "c"))
    ...
    if args.day:
        ...
        for day in range(1, 26):
            ...
            if lang_filter == "python" or lang_filter is None:
                has_lang = has_lang or (year_dir / f"{base}.py").exists()
            if lang_filter == "rust" or lang_filter is None:
                has_lang = has_lang or (year_dir / f"{base}.rs").exists()
            if lang_filter == "zig" or lang_filter is None:
                has_lang = has_lang or (year_dir / f"{base}.zig").exists()
            if lang_filter == "nim" or lang_filter is None:
                has_lang = (
                    has_lang or (year_dir / f"nim_{args.year}_{padded}.nim").exists()
                )
            if lang_filter == "c" or lang_filter is None:
                has_lang = has_lang or (year_dir / f"{base}.c").exists()
